...
timeUsageGroupedSql(plain sql) output:
+-----------+------+------+------------+----+-----+
|    working|   sex|   age|primaryNeeds|work|other|
+-----------+------+------+------------+----+-----+
|not working|female|active|        12.4| 0.5| 10.6|
...
timeUsageGroupedTyped(dataset) output:
+-----------+------+------+------------+----+-----+
|    working|   sex|   age|primaryNeeds|work|other|
+-----------+------+------+------------+----+-----+
|not working|female|active|        12.4| 0.5| 10.6|
...
+-----------+------+------+------------+----+-----+
  ...
  def row(line: List[String]): Row = {
    // val DoubleColumns = line.tail.map(_.toDouble).toSeq
    // _* 标注告诉编译器把 arr 的每个元素当作参数，而不是当作单一的参数传
    // Row(line.head, DoubleColumns: _*)

    val DoubleColumns = line.tail.map(_.toDouble)
    Row.fromSeq(line.head +: DoubleColumns)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    var primary: List[Column] = Nil
    var working: List[Column] = Nil
    var other: List[Column] = Nil
    columnNames.foreach( colunmName => {
      if(colunmName.startsWith("t01") || colunmName.startsWith("t03") || colunmName.startsWith("t11") ||
        colunmName.startsWith("t1801") || colunmName.startsWith("t1803") ){
        primary = new Column(colunmName) :: primary

      } else if (colunmName.startsWith("t05") || colunmName.startsWith("t1805")){
        working = new Column(colunmName) :: working

      } else if (colunmName.startsWith("t02") || colunmName.startsWith("t04") || colunmName.startsWith("t06") ||
        colunmName.startsWith("t07") || colunmName.startsWith("t08") || colunmName.startsWith("t09") ||
        colunmName.startsWith("t10") || colunmName.startsWith("t12") || colunmName.startsWith("t13") ||
        colunmName.startsWith("t14") || colunmName.startsWith("t15") || colunmName.startsWith("t16") ||
        (colunmName.startsWith("t18") && !colunmName.startsWith("t1801")
          && !colunmName.startsWith("t1803") && !colunmName.startsWith("t1805"))){

        other = new Column(colunmName) :: other
      }
    })
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(record=>(record.working, record.sex, record.age))
      .agg(
